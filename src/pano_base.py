...
# traverse_road_consider_reverse_edge, extract_roads_info, 
...
def bfs_helper(x, y, area, pano_id=None, max_level=200, visualize=False, console_log=False):
...
def traverse_panos_by_road(road_name, buffer=500, max_level=400, visualize=True, save=True):
    ...
    for p in tqdm(starts):
        temp = bfs_helper(*p, road_buffer, pano_id=None, max_level=max_level, console_log=True)
...
if __name__ == "__main__":
    ...
    """ 遍历道路 """
    # traverse_panos_by_road('沙河西路',buffer=500, max_level=200)
    import pickle
