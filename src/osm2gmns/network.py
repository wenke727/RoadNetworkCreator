...
def _buildNet(netdata,network_type, POIs,strict_mode, min_nodes, combine, int_buffer, bbox, default_lanes, default_speed):
    network = Network()
...
def getNetFromOSMFile(osm_filename='map.osm', network_type=('auto',), POIs=False, strict_mode=True, min_nodes=1,
    ...
    network = _buildNet(netdata,network_type, POIs,strict_mode, min_nodes, combine, int_buffer, bbox, default_lanes, default_speed)
    return network
