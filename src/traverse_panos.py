...
import pickle
from recognize_intersection import OSM_road_network_parser
...
def cut_and_align_helper(base_line:pd.Series, compared_line:pd.Series):
    ...
    if left != start:
        ...
        if the_foot_point_on_line( base_line.iloc[0], panos_segment ):
            left_foot_point = get_Foot_Point( base_line.iloc[0], compared_line.iloc[left-1], compared_line.iloc[left] )
        if the_foot_point_on_line( base_line.iloc[-1], panos_segment ):
...
def get_related_position(line1:LineString, line2:LineString):
    ...
    # start, end = -1, -1
    # for i in range(n-1):
    #     pos = relation_bet_point_and_line(line1.coords[0], [*coords[i], *coords[i+1]] )
    #     if 0 <= pos <= 1:
    #         start = (i+pos)/(n-1)
    #         break
    #     if i>= n-1:
    ...
    # for i in range(n-1):
    #     pos = relation_bet_point_and_line(line1.coords[-1], [*coords[i], *coords[i+1]] )
    #     if 0 <= pos <= 1:
    #         end = (i+pos)/(n-1)
    #         break
    #     if i>= n-1:
...
def find_matching_panos_path_for_road( road, result, DB_roads=DB_roads, 
...
for i in range(road_osm.shape[0]):
    find_matching_panos_path_for_road( road_osm.iloc[i], result, vis=True, vis_step=False )
