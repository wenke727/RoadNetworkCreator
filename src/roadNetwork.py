...
def map_visualize(df, lyrs = 'p'):
    """
    Draw the geodataframe with the satellite image as the background
    @param: df, GeoDataFrameo
    @param: lyrs, m 路线图; t 地形图; p 带标签的地形图; s 卫星图; y 带标签的卫星图; h 标签层（路名、地名等）
    @return： None
    """
    fig, ax = plt.subplots(1, 1, figsize = (18,12))
    df.plot(color = 'red', ax = ax, zorder = 1)
    # record.plot(color = 'red', ax = ax, zorder = 1)

    [x0, x1], [y0, y1] = plt.xlim(), plt.ylim()
    gap_x, gap_y = (x1-x0), (y1-y0)
    if not 0.4 <= gap_y / gap_x <= 2.5:
        mid_x, mid_y = (x1+x0)/2, (y1+y0)/2
        gap = max( gap_x, gap_y)/1.8
        [x0, y0, x1, y1] = [mid_x - gap, mid_y - gap, mid_x + gap, mid_y + gap]
    else:
        [x0, y0, x1, y1] = [x0-(x1-x0)/10, y0+(y0-y1)/10, x1+(x1-x0)/10, y1-(y0-y1)/10] 
    ...
    ax.imshow( background, extent=[x0, x1, y0, y1], alpha = .5, zorder = 0 )
...
def traverse_road(origin, graph):
    """
    Traverse the road, connect the segement accoding to sptial relationship
    """
    #TODO 光侨路一边的坐标体系；
    ...
    def bfs(queue, visited, res):
        while queue:
            ...
            if node.outdegree == 1:
                ...
                print( "travrese warning: ", node.val, ": ", node.outdegree, "outdegree != 1" )
                for nxt_node in neighbors:
                    print(f'\t 邻接点 {nxt_node.val}, ({nxt_node.indegree}, {nxt_node.outdegree}), ',  [ x.val for x in nxt_node.nxt])
...
def reverse_road_identify(prev_node, cur_node, df_roads):
    if not cur_node.check_0_out_more_2_in():
        return cur_node
    from spatialAnalysis import angle_bet_two_line

    print( "reverse_road_identify: ", cur_node.val )

    # 更新错误方向的线段, 重新合并一下
    nxt_node = None
    angles = {}
    base_line = {'x0': prev_node.x ,'y0':prev_node.y, 'x1':cur_node.x, 'y1':cur_node.y, 'node': prev_node}

    miminum = sys.maxsize
    for node in cur_node.prev:
        if node == prev_node:
            continue
        line = {'x0': cur_node.x ,'y0':cur_node.y, 'x1':node.x, 'y1':node.y, 'node': node}
    ...
    reverse_road_identify(cur_node, nxt_node, df_roads)

def traverse_road_consider_reverse_edge(origin, graph, df_roads):
    res = traverse_road( origin, graph) 
    count = 1
    while len(res) > 1 and graph[res[-1][1]].check_0_out_more_2_in(): #! 判断写少了一个(), 函数没有执行
        print(f"\n--------\n{res[-1][0]},{res[-1][1]}, couting {count}, {graph[res[-1][1]].check_0_out_more_2_in()}\n")
        
        reverse_road_identify(graph[res[-1][0]], graph[res[-1][1]], df_roads)
...
def extract_roads_info(df_roads):
    """
    @DESC: Extract topo information from roads file
    """
...
def create_crawl_point(road_one_way: gpd.GeoDataFrame, visualize = False):
    ...
    df_order_coords = gpd.GeoDataFrame( df_order_coords, geometry = df_order_coords.apply( lambda i: Point(i.x, i.y), axis=1 ) )

    df_order_coords.loc[:,'dis'] = calculate_adj_points_dis(df_order_coords, inplace = True)
    df_order_coords.query( 'dis > 0 ', inplace=True )
    df_order_coords['dis_cum'] = df_order_coords.dis.rolling(df_order_coords.shape[0], min_periods =1).sum()
    ...
    df_order_coords.loc[:, 'coords'] = df_order_coords.apply( lambda x:  bd_coord_to_mc( *ct.wgs84_to_bd09(x.x, x.y) ) , axis=1 )
...
if __name__ == '__main__':
    df_roads = gpd.read_file('/home/pcl/traffic/RoadNetworkCreator_by_View/input/光侨路.geojson')
    df_roads.query("start=='12685799.40,2586251.61' ")
    
    points, graph, zero_indegree_points = extract_roads_info(df_roads)
    res, df = traverse_road_consider_reverse_edge( '12685054.46,2591594.75', graph, df_roads ) 
