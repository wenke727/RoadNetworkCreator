    ...
    if network.POI_list:
        while True:
            try:
                ...
                input()
        writer = csv.writer(outfile)
        writer.writerow(['name', 'poi_id', 'osm_way_id', 'osm_relation_id','building','amenity','geometry','centroid'])
        for poi in network.POI_list:
            line = [poi.name, poi.poi_id, poi.osm_way_id, poi.osm_relation_id, poi.building, poi.amenity, poi.geometry, poi.geometry.centroid]
            writer.writerow(line)